...
namespace Pathfinding {
	...
	public class AIDestinationSetter : VersionedMonoBehaviour {
		...
		void Update () {
			...
			//Calcul de la distance entre le monstre et le loup
			float distance = Mathf.Sqrt( Mathf.Pow(gameObject.transform.position.x - target.position.x, 2) + Mathf.Pow(gameObject.transform.position.y - target.position.y, 2)  );

			if(targetA.activeSelf) targetFinal = target;
			

			//
			if (distance > 7 || targetA.activeSelf == false)
			{
